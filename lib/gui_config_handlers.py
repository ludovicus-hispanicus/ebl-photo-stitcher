...
class ConfigManager:
    ...
    def load_config(app_instance, config_file_path):
        ...
        app_instance.input_folder_var.set(
            loaded_cfg.get("last_folder", defaults["last_folder"]))
        app_instance.ruler_position_var.set(loaded_cfg.get(
            "last_ruler_position", defaults["last_ruler_position"]))
        app_instance.photographer_var.set(loaded_cfg.get(
            "last_photographer", defaults["last_photographer"]))
        app_instance.add_logo_var.set(loaded_cfg.get(
            "last_add_logo", defaults["last_add_logo"]))
        app_instance.logo_path_var.set(loaded_cfg.get(
            "last_logo_path", defaults["last_logo_path"]))
        app_instance.museum_var.set(loaded_cfg.get(
            "last_museum", defaults["last_museum"]))

        if app_instance.measurements_loaded:
            app_instance.use_measurements_var.set(
                loaded_cfg.get("last_use_measurements", False))

        app_instance.gradient_width_fraction = loaded_cfg.get(
